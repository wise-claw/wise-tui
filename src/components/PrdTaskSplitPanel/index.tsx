...
type TaskAiMode = "optimize" | "check";
...
function buildSnapshotAbsoluteDisplayPath(path: string): string {
...
function stripRequirementsIndexSection(markdown: string): string {
  const removed = markdown.replace(/\n?##\s*需求索引（JSON）[\s\S]*?(?=\n##\s|$)/g, "\n");
  return removed.replace(/\n{3,}/g, "\n\n").trim();
}

function stripSectionByHeading(markdown: string, heading: string): string {
...
function clipRuntimeLogText(text: string, maxLen = 12000): string {
  const trimmed = text.trim();
  if (!trimmed) return "";
  return trimmed.length > maxLen ? `${trimmed.slice(0, maxLen)}\n\n...（已截断）` : trimmed;
}

interface ClaudeRuntimeSessionInfo {
  sessionId: string;
  model: string | null;
  cwd: string | null;
  tools: number | null;
...
function parseClaudeRuntimeSessionInfo(rawLine: string): ClaudeRuntimeSessionInfo | null {
...
function formatClaudeRuntimeSessionInfo(info: ClaudeRuntimeSessionInfo): string {
  const lines = [
    "Claude Code 会话已启动",
...
function stripEmbeddedTaskAnchorsFromRequirementMarkdown(markdown: string): string {
  return markdown
    .replace(/\n{0,2}>\s*任务锚点：[^\n]*(?=\n|$)/g, "")
    .replace(/\n{3,}/g, "\n\n")
    .trimEnd();
...
const API_METHOD_OPTIONS = ["GET", "POST", "PUT", "DELETE", "PATCH"] as const;
const TASK_AI_DEFAULT_PROMPT_BY_MODE: Record<TaskAiMode, string> = {
  optimize: [
    "请优化当前任务内容，保持业务目标不变。",
...
function anchorLabelFromTaskId(taskId: string): string {
...
function buildApiSpecTemplate(task: TaskItem): TaskApiSpec {
...
function buildRequestSchemaByMethod(method: typeof API_METHOD_OPTIONS[number], title: string): string {
...
function remapAnchorRangeFromMarkdownToVisible(
  markdown: string,
  anchor: {
    from: number;
    to: number;
...
function remapSplitResultAnchorOffsetsFromMarkdown(markdown: string, result: SplitResult): SplitResult {
...
function includesLoosely(needle: string, haystack: string): boolean {
...
function pickMostRelevantRequirementId(
  task: TaskItem,
...
function parseTaskNumericOrdinal(taskId: string): number | null {
...
function buildExecutableTaskCopiesFromSplitSources(activeResult: SplitResult, sourceTasks: TaskItem[]): TaskItem[] {
...
function defaultTaskConfirmFilterByTasks(tasks: TaskItem[]): TaskConfirmFilter {
...
function buildSelectionAnchorTextHash(
  text: string,
  from: number,
  to: number,
  contextBefore: string,
...
function mergeSplitResultsByAppend(base: SplitResult, incoming: SplitResult): SplitResult {
