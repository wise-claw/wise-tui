...
describe("输入框 race - clearComposerSurfaceSync 后 ignoreNextContentSyncRef 必须归零", () => {
  // 复现"发送一条消息后，复制/粘贴下一条内容时发送按钮变灰"根因：
  //   clearComposerSurfaceSync 同时设 ignoreNext=true（防 setContent("") 回流污染）与 resetting=true。
  //   setContent("") 同步触发的 onContentChange 被 resetting 守卫在 applySemiContentChange 头部
  //   拦截 return，永远到不了消耗 ignoreNext 的分支。onAfterSet 释放了 resetting 却不归零
  //   ignoreNext -> 残留 -> 吞掉下一次用户单次 paste 的 onContentChange -> syncCanSendComposer
  //   不执行 -> canSend 保持 false -> 按钮灰。打字因第二次按键消耗 ignoreNext 而不易察觉，但
  //   单次 paste（只触发一次 onContentChange）会被完全吞掉。

  test("失败模式：onAfterSet 不归零 ignoreNext 时，下次单次 paste 被吞、canSend 保持 false", () => {
    // clearComposerSurfaceSync 完成后状态（bug：ignoreNext 残留 true）
    const resetting = false;     // onAfterSet 已释放
    const ignoreNext = true;     // ← 残留（bug）
    let canSend = false;         // 清空后为 false
    let lastEditorPlain = "";    // 清空
    let promptSynced = "";       // React prompt（debouncedPromptSync 写入）

    // 用户 paste "hello" -> Tiptap 同步 onContentChange -> applySemiContentChange
    const pastedPlain = "hello";
    // applySemiContentChange 头部守卫顺序：semiEditorReady -> resetting -> skipContentSync -> ignoreNext
    let enteredNormalPath = false;
    if (resetting) {
      // 被吞
    } else if (ignoreNext) {
      // 一次性消耗后 return：不调 syncCanSendComposer、不更新 lastEditorPlain、不调 debouncedPromptSync
    } else {
      enteredNormalPath = true;
      lastEditorPlain = pastedPlain;
      canSend = pastedPlain.trim().length > 0;
      promptSynced = pastedPlain;
    }
    expect(enteredNormalPath).toBe(false);   // paste 被挡在 ignoreNext 分支
    expect(canSend).toBe(false);             // ← 失败模式：按钮灰
    expect(lastEditorPlain).toBe("");        // paste 内容未同步到权威 ref
    expect(promptSynced).toBe("");           // React prompt 未更新
  });

  test("修复后：onAfterSet 归零 ignoreNext，paste 正常翻 canSend 并同步 React 状态", () => {
    const resetting = false;
    const ignoreNext = false;    // ← 修复：onAfterSet/onGiveUp 归零
    let canSend = false;
    let lastEditorPlain = "";
    let promptSynced = "";
    ...
    expect(enteredNormalPath).toBe(true);
    expect(canSend).toBe(true);              // ← 修复：按钮可点
    expect(lastEditorPlain).toBe("hello");   // paste 内容已同步（发送时不会漏发）
    expect(promptSynced).toBe("hello");
  });

  test("onGiveUp 路径同样归零：editor 未就绪放弃 setContent 后 ignoreNext 不残留", () => {
    // clearComposerSurfaceSync 的 onGiveUp（editor 48 帧未就绪）也必须归零 ignoreNext，
    // 否则 resetting 释放后残留的 ignoreNext 仍会吞下次输入。
    const resetting = false;     // onGiveUp 已释放 resetting
    const ignoreNext = false;    // ← 修复：onGiveUp 归零
    let canSend = false;
    const pastedPlain = "hello";
    if (!resetting && !ignoreNext) {
      canSend = pastedPlain.trim().length > 0;
    }
    expect(canSend).toBe(true);
  });

  test("会话切换兜底：ignoreNext 必须与 pending/resetting 一同归零，防跨会话残留", () => {
    // 旧会话 clearComposerSurfaceSync 残留 ignoreNext=true，切到新会话后新会话首次 paste 不应被吞。
    let pending = 1;
    let resetting = true;
    let ignoreNext = true;
    // 会话切换 effect（与 pendingSetContentRef/composerResettingRef 同处归零）
    pending = 0;
    resetting = false;
    ignoreNext = false;          // ← 修复：会话切换兜底归零
    // 新会话首次 paste
    const pastedPlain = "新会话首条";
    let canSend = false;
    if (!resetting && !ignoreNext) {
      canSend = pastedPlain.trim().length > 0;
    }
...
describe("输入框 race - scheduleComposerSetContent 统一归零 ignoreNext 覆盖所有路径", () => {
  // 修复演进：从「clearComposerSurfaceSync 单点归零」优化为「scheduleComposerSetContent 的
  //   onAfterSet/onGiveUp 统一归零」。这样非 resetting 路径（setPlainAndCursor / @补全插入 /
  //   restoreComposerSurface / applyStarterPrompt / displayPlain effect）若 shouldSkipStaleComposerSetContent
  //   判定 skip（editor 已聚焦且已有目标内容，不调 ai.setContent、不触发 onContentChange），
  //   ignoreNext 也不再残留 -- 否则下次 paste 被吞、漏发内容（虽 canSend 已 true 不致灰，但发送时
  //   读 lastEditorPlainRef 旧值漏发 paste）。

  test("setPlainAndCursor skip 路径：setContent 被 skip 不触发回流，onAfterSet 仍归零 ignoreNext，下次 paste 不漏发", () => {
    // @补全插入后 editor 已有目标内容 -> shouldSkipStaleComposerSetContent 返回 true -> 不 setContent
    const setContentCalled = false;
    const onContentChangeFired = false; // skip 不触发回流，ignoreNext 不会被被动消耗
    let ignoreNext = true;              // setPlainAndCursor 设
    let lastEditorPlain = "已插入的内容";
    let canSend = true;                 // settle 的 syncCanSendComposer(plain) 已翻 true

    // scheduleComposerSetContent 内部 onAfterSet（settle 后）主动归零
    if (!setContentCalled && !onContentChangeFired) {
      ignoreNext = false;               // ← 统一归零（优化点）
    }

    // 用户 paste "补充" -> onContentChange
    const pastedPlain = "补充";
    let enteredNormal = false;
    if (!ignoreNext) {                  // ignoreNext 已归零，不被吞
      enteredNormal = true;
      lastEditorPlain = lastEditorPlain + pastedPlain;
      canSend = lastEditorPlain.trim().length > 0;
    }
    expect(enteredNormal).toBe(true);
    expect(canSend).toBe(true);
    expect(lastEditorPlain).toBe("已插入的内容补充"); // paste 内容正确拼接，不漏发
  });

  test("失败模式（优化前）：skip 路径不归零 ignoreNext，下次 paste 被吞、内容漏发", () => {
    const setContentCalled = false;
    let ignoreNext = true;              // setPlainAndCursor 设
    let lastEditorPlain = "已插入的内容";
    // 优化前：仅 clearComposerSurfaceSync 单点归零，此 skip 路径漏 -> ignoreNext 残留 true
    ...
    expect(enteredNormal).toBe(false);               // paste 被吞
    expect(lastEditorPlain).toBe("已插入的内容");     // ← 漏发 paste 内容（bug）
  });
});
