...
type MaterialTheme = "blue" | "green" | "orange";
...
interface CanvasNodeItem {
  ...
  stageTask?: string;
  employeeId?: string;
  /** 多条阶段成果（名称 + Markdown 要求）；非空时写入工作流图并随派发注入强约束 */
  stageSuccessCriteria?: WorkflowStageOutcomeCriterion[];
  /** 多选；每项 `节点id#成果索引`，与 workflowGraphRuntime 中约定一致 */
  stageTaskBasisRefs?: string[];
  /** @deprecated 旧版单选；读取时并入 stageTaskBasisRefs */
...
const MATERIAL_NODE_WIDTH = 204;
const MATERIAL_NODE_HEIGHT = 78;
const FLOW_NODE_WIDTH = 128;
const FLOW_NODE_HEIGHT = 52;
const EMPLOYEE_NODE_HEIGHT = 124;
const EMPLOYEE_NODE_HEIGHT_WITH_ACCEPTANCE = 148;
const EMPLOYEE_NODE_HEIGHT_STAGE_SUCCESS_EXTRA = 22;
const EMPLOYEE_NODE_HEIGHT_TASK_BASIS_EXTRA = 18;
...
function createDefaultCanvasSnapshot(): CanvasSnapshot {
  return {
    nodes: [],
    edges: [],
  };
...
function workflowGraphToCanvasSnapshot(graph: WorkflowGraph | null | undefined): CanvasSnapshot {
...
function getMaterialNodeStyle(theme: MaterialTheme) {
...
let WORKFLOW_X6_NODE_REGISTERED = false;

function ensureWorkflowX6Nodes() {
  if (WORKFLOW_X6_NODE_REGISTERED) return;
  Graph.registerNode(
    "wise-material-card",
    {
      inherit: "rect",
  ...
  WORKFLOW_X6_NODE_REGISTERED = true;
}
...
function setPortVisible(node: X6Node, portId: string, visible: boolean) {
  node.setPortProp(portId, "attrs/circle/style/visibility", visible ? "visible" : "hidden");
}

function setPortColor(node: X6Node, portId: string, connected: boolean) {
...
function isPortConnected(graph: Graph, node: X6Node, portId: string) {
...
function refreshNodePorts(graph: Graph, node: X6Node, showAll: boolean) {
...
function buildEmployeeNodeSummary(node: Partial<CanvasNodeItem>, employeeNameById: Record<string, string>) {
...
function getEmployeeNodeHeight(node: Partial<CanvasNodeItem>): number {
...
function createGraphNodeFromSnapshotNode(node: CanvasNodeItem, employeeNameById: Record<string, string> = {}): any {
