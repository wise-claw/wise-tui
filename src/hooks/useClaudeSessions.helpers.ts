...
export function sessionHasVisibleStreamProgress(session: ClaudeSession): boolean {
  ...
  return session.messages.some((m) => {
    ...
    if (m.role === "system" && m.content.trim().length > 0) {
      if (
        m.content.trim() === "Cursor CLI 执行中…" ||
        m.content.trim() === "Cursor Agent 执行中…" ||
        m.content.trim() === "Cursor SDK 执行中…"
      ) {
        return false;
      }
      return true;
  ...
  for (let i = 0; i < copy.length; i++) {
    ...
    if (item) {
      ...
      copy[i] = {
        ...
        // 才能把它分到「今天/昨天」；否则可能被 Math.min 压回「更早日期」并随机错位。
        createdAt: s.messages.length > 0
          ? Math.min(s.createdAt, item.updatedAtMs)
          : Math.max(s.createdAt, item.updatedAtMs),
      };
    } else {
