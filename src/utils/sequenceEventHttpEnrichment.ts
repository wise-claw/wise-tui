...
function applyObservedHttp(
  ev: SequenceEvent,
  opts: { id: string; subtitle: string; detail: string; source: "fcc" | "llm_proxy" },
): void {
  ev.id = opts.source === "fcc" ? `fcc-api-${opts.id}` : `llm-api-${opts.id}`;
...
/** 将 FCC / LLM 代理记录填入已存在的 `api_request` 占位。 */
export function enrichSequenceEventsWithObservedHttp(
  ...
  if (fcc.length === 0 && llm.length === 0) {
    return { events: [...events], unusedFccTraces: [] };
  }
  ...
  return { events: copy, unusedFccTraces: unusedFcc };
}
