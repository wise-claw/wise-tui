/**
 * Kitty / fixterms CSI-u 键盘协议序列若未被 WASM 消费，会落成可见的 `6;5u` 等碎片。
 * 在 PTY 输出写入 ghostty 前剥离，避免污染屏幕与行编辑。
 */
...
/** 关闭 ghostty-web 终端侧的增强键盘模式（不影响 PTY 内 shell 状态）。 */
...
/** 经输出队列串行写入，避免与 PTY replay 并发抢占 ghostty 解析状态。 */
