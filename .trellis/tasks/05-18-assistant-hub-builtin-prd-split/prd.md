...
- **D16 ② 落盘执行回到编排页主动作**。
  ...
  - 编排页取消低价值的左侧解释栏,改为顶部摘要 + 主 wave 画布 + fan-out 映射区。
  - 波次调整主交互改为拖拽任务到既有目标波次;按钮只保留为辅助快捷操作。
  - 编排画布只展示“并行框”和“串行链路”,不提供“新 Pool”临时概念;当所有任务仍在并行框时,仍必须提供固定“串行链路”落点,用于把任务恢复为后置串行步骤。
  - 编排页必须展示 PRD requirement → task → agent → sourceRefs 的追溯链路。
  - 右侧状态监控展示具体子代理名称,禁止只展示笼统 `trellis-implement`。
  - 同波次文件/资源冲突必须以警告形式展示;拖拽若会制造冲突,必须弹出二次确认,不能直接落盘。
  - 缺少 `sourceRefs` 时不在任务卡主信息位显示“待声明文件触点”;只在状态监控或质量提示里暴露元数据缺口。
  - 依赖分析必须补足常见工程语义,例如 JWT/令牌/请求拦截类任务默认后置于登录注册页面/认证表单任务。

- **D16 ③ 拆分阶段输出编排所需元数据**。
  - splitter 输出必须包含 `sourceRefs`,用于展示任务会创建/修改/检查的文件或目录。
  - splitter 输出应包含具体 `agentHint`,例如 `DBA-Agent` / `Config-Agent` / `API-Agent` / `Frontend-Coder`。
  - splitter 输出应包含 `conflictHints`,用于提示文件锁、schema、接口契约、配置资源等并行冲突。
