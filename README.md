...
- 自动发现本机原生运行的 Codex / DeepSeek Harness / Cursor ACP 会话（`~/.codex/sessions`、`~/.dsh/sessions`、`~/.cursor/acp-sessions`），以及 Wise 自己落在 `~/.wise/cursor-runs` 的 Cursor 转录，在左栏带引擎徽标展示，并可直接以原生会话 id 续接执行。
- Composer 支持富文本、文件 / 图片上下文、常用语、语音输入和快捷指令。
- 可在「工作台配置 → 新建会话」指定助手回复语言（默认跟随引擎 / 简体中文 / English / 日本語），代码、命令与引用原文保持原样。
...
在「产物」中心选择「画布」，或从仓库文件树打开 `.html`、`.htm`、`.svg`、`.md`、`.markdown` 文件，会默认显示画布。顶部可切换「源码」编辑；切回画布可查看当前内容（包括未保存的修改）。支持自适应、桌面（1280px）、平板（768px）、手机（390px）宽度，支持 50%–200% 缩放和展开查看；点击「刷新画布」会重新读取依赖资源并运行交互脚本。

画布适合 HTML 页面、交互图表和 SVG。自动加载仓库内通过 link 引用的 CSS、通过 src 引用的 JavaScript、图片，以及 CSS url() 中的图片和字体；相对路径按所属文件目录解析，根路径按仓库根目录解析。HTTPS 资源需联网且受目标服务的跨域限制。CSS @import、响应式图片 srcset 和模块脚本的相对 import 暂需合并、内嵌或预先打包，JSX/TSX 项目需先构建。缺失、越界或超限资源会显示详细提示。依赖资源限制为单文件 4MB、累计 16MB、64 个二进制资源及 32 个样式文件。脚本在独立沙箱内运行，不能访问 Wise 页面、登录存储或原生接口；超大文件保留源码查看。切走标签会释放画布，返回时重新运行。

聊天中的助手回复提供「画布查看」：默认以文档画布呈现方案全文（支持 Markdown 表格和 Mermaid 流程图），回复中的完整 HTML / SVG 代码块可在「查看内容」中切换为可交互的页面或设计图。无需先保存为文件。未结束的代码块不会运行；JSX / TSX 仍需要先构建。画布按用户点击打开，不会在生成过程中自动弹出。

「产物 → 画布 · 页面与方案」同时收录网页、SVG 和 Markdown 方案，Markdown 分类仍保留。方案文件默认进入文档画布，可切换源码编辑。聊天画布是当前回复的查看视图，不能直接保存编辑；需要持久化时让助手生成仓库文件。
